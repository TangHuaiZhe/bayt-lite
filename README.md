...
1. FFmpeg 先从本地视频提取 M4A 音频，再将音频转成单声道 16 kHz MP3，并按 10 分钟切片。
2. 每个切片依次完成本机 MLX Whisper 英文转写和 DeepSeek 分批翻译，完成一份就将双语结果和进度保存到本机。
3. 听译台按照英文句末标点合并短片段，并保留跨切片的未完句；翻译中断后，本地任务可从下一个未完成切片继续。
...
- 重试本地任务会复用原始音频和已完成切片；重试链接任务会重新解析节目地址并重新下载音频。
