...
听译台是一个面向个人使用的本地播客听译工具：导入英文音频，在本机生成带时间戳的英文字幕，再通过 DeepSeek API（开放平台接口）翻译和总结，并让字幕高亮始终跟随播放进度。

## 功能

- 导入本地音频、音频直链、Apple Podcasts（苹果播客）单集链接或 RSS（播客订阅源）单集
...
- 自动保存每集播放进度
- 本机 JSON（轻量数据文件）持久化、处理进度、任务取消和完整删除
- 内置无需密钥即可播放的演示单集
- 桌面端和移动端响应式布局
...
3. 听译台按照英文句末标点合并短片段，再把完整句子按批次发送给 DeepSeek，获得结构化中文翻译。
