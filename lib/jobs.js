...
export async function processJob(id, { signal } = {}) {
  try {
    ...
    for (let index = 0; index < chunks.length; index += 1) {
      await setProgress(id, "processing", 12 + Math.round((index / chunks.length) * 43), `正在转写第 ${index + 1}/${chunks.length} 段`, signal);
      ...
      timeOffset += await duration(chunks[index], signal);
    }

    const sentenceSegments = mergeSegmentsBySentence(segments);
    await setProgress(id, "processing", 58, "正在翻译字幕", signal);
    const translated = await translateSegments(sentenceSegments, async (done, total) => {
      await setProgress(id, "processing", 58 + Math.round((done / total) * 36), `正在翻译 ${done}/${total} 句`, signal);
    }, { signal });
